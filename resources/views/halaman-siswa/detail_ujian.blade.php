...
<div class="col-md-12">
	<div class="box box-primary">
		...
		<div class="box-body">
			<div class="row">
				<div class="col-sm-12">
					<h1 style="margin:  0 0 0 0; color: #292e38; font-size: 24pt;">{{ $soal->mapel->nama_mapel }} kelas {{$kelas['0']['kelas']}} {{$kelas['0']['tipe_kelas']}}</h1>
...
<div id="specialstuff" class="row" style="display: none; overflow-y: scroll !important;">
	<div style="height: 40px; background: #0419d0; color: #fff; margin-bottom: 10px">
		<p style="padding-top: 8px; padding-left: 15px; font-weight: bold;">RolasXam | Examination Rolas Apps for Education</p>
	</div>
	<div class="col-sm-9">
		<div class="box box-primary color-palette-box" style="overflow-y: scroll;">
			...
			<div class="box-body">
				<div id="wrap-soal">
					...
					{!! $data->pila ? '<div class="jawab" soal-id="'.$data->ulangans_id.'" data-id="'.$data->id.'" data-jawab="A/'.$data->id.'/'.Auth::user()->id.'">
						<table width="100%">
							<tr>
					...
					{!! $data->pilb ? '<div class="jawab" soal-id="'.$data->ulangans_id.'" data-id="'.$data->id.'" data-jawab="B/'.$data->id.'/'.Auth::user()->id.'">
						<table width="100%">
							<tr>
					...
					{!! $data->pilc ? '<div class="jawab" soal-id="'.$data->ulangans_id.'" data-id="'.$data->id.'" data-jawab="C/'.$data->id.'/'.Auth::user()->id.'">
						<table width="100%">
							<tr>
					...
					{!! $data->pild ? '<div class="jawab" soal-id="'.$data->ulangans_id.'" data-id="'.$data->id.'" data-jawab="D/'.$data->id.'/'.Auth::user()->id.'">
						<table width="100%">
							<tr>
					...
					{!! $data->pile ? '<div class="jawab" soal-id="'.$data->ulangans_id.'" data-id="'.$data->id.'" data-jawab="E/'.$data->id.'/'.Auth::user()->id.'">
						<table width="100%">
							<tr>
				...
				<table width="100%">
					<tr>
						...
						<td>
							...
							<button type="button" class="btn pull-right" id="kirim" style="background-image: linear-gradient(to right, #1523f3 , #0495c1); border: none; color: #fff;"><i class="fa fa-paper-plane" aria-hidden="true" data-id="{{$data->id}}"></i> Kirim Hasil Ujian</button>
						</td>
					</tr>
				</table>
...
<script>
	...
	$(document).ready(function() {
		...
		$(document).on('click', '.no_soal_essay', function() {
			const detailsoalessays_id = $(this).data('id');
			$.ajax({
				url: "{{ url('ujian/get-detail-essay') }}",
				type: "GET",
				data: {
					detailsoalessays_id: detailsoalessays_id
				},
		...
		$('.no_soal').click(function() {
			...
			$.ajax({
				type: "GET",
				url: "{{ url('ujian/get-soal') }}/" + id_soal,
				success: function(data) {
					$('#wrap-soal').html(data);
				}
			})

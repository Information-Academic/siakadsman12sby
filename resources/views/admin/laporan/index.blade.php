			...
			<table class="table table-condensed">
				...
				<tbody>
					...
					<tr>
						...
						<td>{{ ucwords(strtolower($data_kelas->kelas)) }} {{ ucwords(strtolower($data_kelas->tipe_kelas)) }}</td>
						<td class="text-center">{{ number_format($data_kelas->siswa->count(),0,'.','.').' siswa' }}</td>
						<td align="center"><a href="{{ url('/laporan/detail-kelas/'.$data_kelas->id) }}" class="btn btn-success btn-xs">Detail</a></td>
					</tr>
					@endforeach
					@else
					<tr>
